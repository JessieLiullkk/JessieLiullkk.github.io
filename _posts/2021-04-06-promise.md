...
#### 1. Generator定义
##### 1-1. function关键字与函数名之间有一个星号；
##### 1-2. 函数体内部使用yield表达式，定义不同的内部状态（yield在英语里的意思就是“产出”）。
...
#### 4. next()/throw()/return()的共同之处
##### 4-1. next()是将yield表达式替换成一个值。
```markdown
...
// 替换成 let result = 1;
```
##### 4-2. throw()是将yield表达式替换成一个throw语句。
```markdown
gen.throw(new Error('出错了')); // Uncaught Error: 出错了
// 相当于将 let result = yield x + y
// 替换成 let result = throw(new Error('出错了'));
```
##### 4-3. return()是将yield表达式替换成一个return语句。
```markdown
